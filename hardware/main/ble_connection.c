...
// 消息队列相关定义
#define GIF_QUEUE_LENGTH 10
#define GIF_QUEUE_ITEM_SIZE sizeof(char)

// 消息队列句柄
static QueueHandle_t gif_queue = NULL;
...
static void heart_gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_READ_EVT:
        ...
        // 打印读取请求
        ESP_LOGI(GATTS_TAG, "Heart rate read request, sending value: %d", heart_rate_val[1]);
        break;
...
static void auto_io_gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_WRITE_EVT:
        ...
        // 处理收到的数据，将字符直接发送到消息队列切换GIF
        if (param->write.len > 0) {
            // 获取收到的字符
            char ch = param->write.value[0];
            ESP_LOGI(GATTS_TAG, "Received character: %c, sending to queue", ch);
            
            // 发送消息到GIF切换队列
            if (xQueueSend(gif_queue, &ch, portMAX_DELAY) != pdTRUE) {
                ESP_LOGE(GATTS_TAG, "Failed to send GIF switch command to queue");
            }
        }
...
static void gif_process_task(void* param)
{
    ESP_LOGI(GATTS_TAG, "GIF process task started");
    
    char gif_char;
    while (1) {
        // 等待接收消息队列中的GIF切换命令
        if (xQueueReceive(gif_queue, &gif_char, portMAX_DELAY) == pdTRUE) {
            ESP_LOGI(GATTS_TAG, "Received GIF switch command: %c", gif_char);
            
            // 调用函数切换GIF图像
            esp_err_t ret = display_switch_gif_by_char(gif_char);
            if (ret != ESP_OK) {
                ESP_LOGE(GATTS_TAG, "Failed to switch GIF: %s", esp_err_to_name(ret));
            }
        }
    }
}
...
esp_err_t ble_connection_init(void)
{
    ...
    // 设置设备名称为 "desk_toy"
    ret = esp_ble_gap_set_device_name("desk_toy");
    if (ret) {
        ESP_LOGE(GATTS_TAG, "%s set device name failed: %s", __func__, esp_err_to_name(ret));
        return ret;
    }
    ...
    // 创建GIF切换消息队列
    gif_queue = xQueueCreate(GIF_QUEUE_LENGTH, GIF_QUEUE_ITEM_SIZE);
    if (gif_queue == NULL) {
        ESP_LOGE(GATTS_TAG, "Failed to create GIF queue");
    ...
    // 创建GIF处理任务
    xTaskCreate(gif_process_task, "GIF Process", 8 * 1024, NULL, 5, NULL);
