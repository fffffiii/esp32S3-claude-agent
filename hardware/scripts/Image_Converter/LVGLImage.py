    ...
    def convert(self):
        ...
        for f in self.files:
            if self.cf in (ColorFormat.RAW, ColorFormat.RAW_ALPHA):
                ...
                # 检查文件扩展名，如果是GIF，只支持RAW格式
                ext = path.splitext(f)[1].lower()
                if ext == '.gif':
                    raise BaseException(f"GIF files only support RAW or RAW_ALPHA color formats: {f}")
...
def main():
    ...
    if path.isfile(args.input):
        ...
        # 查找PNG和GIF文件
        files = list(Path(args.input).rglob("*.[pP][nN][gG]")) + list(Path(args.input).rglob("*.[gG][iI][fF]"))
    else:
